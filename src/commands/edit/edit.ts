import { sendDirectMessage, MessageContent, Message } from "../../../deps.ts";
...
createSubcommand("edit", {
  ...
  execute: async function (message, args) {
    if (args.level < 0 || args.level > 8) return sendDMOrResponse(message, "The world level must be between 0-8");
...
createSubcommand("edit", {
  ...
  execute: async function (message, args) {
    if (args.level < 1 || args.level > 60) return sendDMOrResponse(message, "The adventurer rank must be between 1-60");
...
createSubcommand("edit", {
  ...
  execute: async function (message, args) {
    if (!characters.has(args.character)) return sendDMOrResponse(message, "Invalid character name.").catch(console.log);

    if (args.level < 0 || args.level > 6)
      return sendDMOrResponse(message, "Invalid character level.").catch(console.log);

    const settings = await db.users.get(message.author.id);
    if (!settings) return sendDMOrResponse(message, "No profile found.").catch(console.log);
    ...
    if (settings.characters.some((c) => c.name === args.character)) {
      ...
      settings.characters.push({ name: args.character, constellationLevel: args.level });
    }
